...
class Page(BaseModel):
    """
    Represents a single page in a document or drawing set.
    ...
    page_number: int = Field(
        ...,
        ge=1,
        description="The page number, must be a positive integer starting from 1.",
        example=1,
    )
...
class Measure(Cue):
    ...
    callout_type: Literal[CueType.MEASURE] = CueType.MEASURE
...
class Document(BaseModel):
    """
    Represents a document consisting of multiple pages.

    Attributes:
    ----------
    - pages (List[Page]): A list of pages that make up the document.
      Each page contains detailed information relevant to the document structure.
    """

    pages: List[Page] = Field(
        ...,
        description="A list of pages that belong to the document.",
        min_items=1,
    )
...
class MechanicalComponentPage(Page):
    """
    Represents a mechanical component derived from a sheet material, with associated
    ...
    - projection_method (ProjectionMethod): The projection method used in the drawing.
    - dimensions (List[Callout]): Dimensional specifications for the component.
    - measures (List[Measure]): Measurement details for the component.
    - threads (List[Thread]): Thread specifications.
    - bores (List[Bore]): Bore specifications.
    - chamfers (List[Chamfer]): Chamfer specifications.
    - roughnesses (List[Roughness]): Surface roughness details.
    - gdnts (List[GDnT]): Geometric dimensioning and tolerancing details.
    - input_geometry (Geometry): The input geometry or shape of the material.
    - output_geometry (Geometry): The final geometry or shape after processing.
    - processes (List[Process]): List of manufacturing processes applied.
    ...
    general_tolerances: GeneralTolerances = Field(
        ...,
        description="General tolerance specifications for the component.",
    )
    general_roughness: Roughness = Field(
        ...,
    ...
    dimensions: List[Cue] = Field(
        default_factory=list,
        description="Dimensional specifications provided in the drawing.",
    )
    measures: List[Measure] = Field(
        default_factory=list,
        description="Measurement details for the component.",
    )
    ...
    bill_of_material: List[BillOfMaterialRow] = Field(
        default_factory=list,
        description="Bill of materials for the component, listing parts and quantities.",
    )
    revision_table: List[RevisionTableRow] = Field(
        default_factory=list,
